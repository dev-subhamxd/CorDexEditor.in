...
const LANG = {
  ...
  java: { label:"Java",       mime:"text/x-java",  cls:"lang-java", icon:"J", run:"none" },
};
const ALLOWED_EXT = Object.keys(LANG);

const QUICK_FILES = ["index.html","style.css","script.js","main.py","query.sql","Program.cs","Main.kt","Main.java"];
...
function switchTab(name){
  ...
  if(name === "editor"){
    closeFileSwitchMenu();
    // CodeMirror doesn't repaint while its container is hidden (display:none),
    // so anything that changed the doc while this tab was in the background
    // (deleting the open file/project, etc.) can leave stale, disconnected
    // text on screen until we force a re-measure here.
...
async function runPython(file){
  ...
  try{
    ...
    // input() has no terminal to read from in a browser, so route it
    // through a native prompt dialog. Returning undefined signals EOF
    // (raises a normal Python EOFError) if the user cancels the prompt.
    py.setStdin({
      stdin: () => {
        const val = window.prompt(`${file.name} is asking for input:`);
...
/* ---- C# / Kotlin / Java (no in-browser runtime) ---- */
const NO_RUNTIME_HINT = {
  cs:   "copy it into an online compiler (like .NET Fiddle) or your own local toolchain to run it",
  kt:   "copy it into the Kotlin Playground or your own local toolchain to run it",
  java: "copy it into BlueJ (or another Java IDE / the javac + java command line) to compile and run it",
};
...
function runUnsupported(file){
  ...
  logLine("info", `You can still write and organize ${file.name} — ${NO_RUNTIME_HINT[ext] || "copy it into an online compiler or your own local toolchain to run it"}.`);
}
